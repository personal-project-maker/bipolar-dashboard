...
def safe_get_worksheet(tab_name: str):
    ...
    try:
        ...
        return workbook.add_worksheet(title=tab_name, rows=200, cols=20)
...
def load_weights_from_sheet() -> dict[str, float]:
    ...
    if not values or len(values) == 1:
        save_weights_to_sheet(DEFAULT_WEIGHTS)
    ...
    for _, row in df.iterrows():
        ...
        if code in weights:
            weights[code] = float(pd.to_numeric(row.get("weight"), errors="coerce") or weights[code])
    return weights


def save_weights_to_sheet(weights: dict[str, float]) -> None:
    worksheet = safe_get_worksheet(SETTINGS_TAB)
...
def render_weights_editor(weights: dict[str, float]) -> dict[str, float]:
    ...
    with save_col:
        if st.button("Save weights", type="primary"):
            save_weights_to_sheet(updated)
            st.success("Weights saved.")
    with reset_col:
        if st.button("Reset to defaults"):
            save_weights_to_sheet(DEFAULT_WEIGHTS.copy())
            st.success("Weights reset to defaults. Refresh to reload them into the inputs.")
    return updated
